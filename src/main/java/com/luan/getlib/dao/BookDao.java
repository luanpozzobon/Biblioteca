...
public class BookDao {
    public static boolean saveBook(Book book){
        try(Connection conn = Database.getConnection();
            ...
            st.setString(5, book.getParentalRating());
    ...
    public static boolean updateBook(Book book){
        try(Connection conn = Database.getConnection();
            PreparedStatement st = conn.prepareStatement("UPDATE books SET amount = ?, value = ? WHERE id = ?")){
    ...
    public static Book findById(int id){
        try(Connection conn = Database.getConnection();
            ...
            if(rSet.next()) return new Book(rSet.getInt("id"),
                                            rSet.getString("title"),
                                            rSet.getString("genre"),
                                            rSet.getInt("amount"),
                                            rSet.getDouble("value"),
                                            rSet.getString("parentalRating"));
    ...
    public static List<Book> findByTitle(String title){
        try(Connection conn = Database.getConnection();
            PreparedStatement st = conn.prepareStatement("SELECT id, title FROM books WHERE title LIKE ? ORDER BY title ASC")){
            ...
            while(rSet.next()){
                books.add(new Book(rSet.getInt("id"),
                                   rSet.getString("title")));
            }
            return books;
        } catch(SQLException e){
    ...
    public static List<Book> getAllBooks(){
        try(Connection conn = Database.getConnection();
            PreparedStatement st = conn.prepareStatement("SELECT id, title FROM books ORDER BY title ASC")){
            ResultSet rSet = st.executeQuery();
